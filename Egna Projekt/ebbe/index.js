window.content = function(container) {
  ...
  function show(target, direction) {
    ...
    // Tell to CSS that we're ready to animate transitions
...
content.Signal = function() {
  this.listeners = [];
};

content.Signal.prototype.add = function(callback) {
  this.listeners.push(callback);
};

content.Signal.prototype.remove = function(callback) {
  var i = this.listeners.indexOf(callback);

  if (i >= 0) this.listeners.splice(i, 1);
};

content.Signal.prototype.dispatch = function() {
...
var k = content(document.querySelector(".content"));
